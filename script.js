...
const verifiedRecords = Object.fromEntries([
  ...
  ["John Ratliff",154,"Sandusky Bay â€” May 24",33.75,"Channel catfish","Sandusky Bay â€” May 24"],["Jon Myers",146.5,"Sandusky Bay â€” May 24",32.5,"Blue catfish","Hoover …5177 tokens truncated…ayout"><div class="article-story">${paragraphs}<aside class="article-season-note"><strong>Fact-check note</strong><p>${escapeHtml(factNote)}</p></aside><div class="article-sources"><span>Sources</span>${sourcesHtml}</div></div><aside class="article-results"><span class="eyebrow dark">AT A GLANCE</span><h2>${escapeHtml(sidebarTitle)}</h2><ol>${sidebarHtml}</ol><div class="article-big-fish"><span>${escapeHtml(highlight[0])}</span><strong>${escapeHtml(highlight[1])}</strong><small>${escapeHtml(highlight[2])}</small></div><a class="button primary" href="${escapeHtml(sources[0][1])}" target="_blank" rel="noopener">Open primary source â†—</a></aside></div>`;
}
